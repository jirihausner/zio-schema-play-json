...
object PlayJsonJsoniterCodecJVMSpec extends ZIOSpecDefault {

  def spec: Spec[TestEnvironment, Any] =
    suite("PlayJsonJsoniterCodec JVM Spec")(
      readingSuite,
  ...
  private def assertReadsJson[A](schema: Schema[A], value: A, json: String) = {
    implicit val format: Format[A] = PlayJsonJsoniterCodec.schemaFormat(schema)
    val either                     =
      try {
