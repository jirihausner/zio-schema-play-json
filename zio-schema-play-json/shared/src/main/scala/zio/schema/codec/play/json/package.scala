...
package object json {
  ...
  implicit val schemaJsObject: Schema[JsObject] =
    Schema.dynamicValue
      .transform(toJsObject, (obj: JsObject) => fromJsValue(obj))
      .annotate(directDynamicMapping())

  implicit val schemaJsNumber: Schema[JsNumber] =
    Schema.dynamicValue
      .transformOrFail(toJsNumber, (number: JsNumber) => Right(fromJsValue(number)))
      .annotate(directDynamicMapping())
  ...
  private[play] def toJsValue(dv: DynamicValue): JsValue =
    dv match {
      ...
      case DynamicValue.Sequence(values)               => JsArray(values.map(toJsValue).toIndexedSeq)
      case DynamicValue.Dictionary(_)                  =>
        throw new Exception("DynamicValue.Dictionary is not supported")
      case DynamicValue.SetValue(values)               => JsArray(values.map(toJsValue).toIndexedSeq)
      ...
      case DynamicValue.Tuple(left, right)             => JsArray(Seq(toJsValue(left), toJsValue(right)))
  ...
  private[play] def toJsObject(dv: DynamicValue): JsObject =
    dv match {
      case DynamicValue.Record(_, values) => JsObject(values.map { case (k, v) => (k, toJsValue(v)) })
      ...
      case DynamicValue.Singleton(_)                   => JsObject.empty
      case DynamicValue.SomeValue(value)               => toJsObject(value)
      case DynamicValue.NoneValue                      => JsObject.empty
  ...
  private[play] def toJsNumber(dv: DynamicValue): Either[String, JsNumber] = {
    dv match {
      case DynamicValue.Primitive(value, standardType) =>
        val json: JsValue = standardType.asInstanceOf[StandardType[_]] match {
          case StandardType.UnitType           => JsObject.empty
          case StandardType.StringType         => JsString(value.asInstanceOf[String])
          case StandardType.BoolType           => JsBoolean(value.asInstanceOf[Boolean])
          case StandardType.ByteType           => JsNumber(value.asInstanceOf[Byte].toInt)
        ...
        json match {
          case number: JsNumber => Right(number)
          case _                => Left(s"Failed to parse ${Json.stringify(json)} as number")
        }
      case _                                           =>
